...
def test_max(t):
    assert minitorch.max(t, 0) == max([t[0, 0, 0], t[1, 0, 0]])
    assert minitorch.max(t, 1) == max([t[0, 0, 0], t[0, 1, 0], t[0, 2, 0]])
    assert minitorch.max(t, 2) == max([t[0, 0, 0], t[0, 0, 1], t[0, 0, 2], t[0, 0, 3]])

    jitter = t + minitorch.rand((2, 3, 4)) * 1e-2
    minitorch.grad_check(lambda a: minitorch.max(a, 0), jitter)
    minitorch.grad_check(lambda a: minitorch.max(a, 1), jitter)
    minitorch.grad_check(lambda a: minitorch.max(a, 2), jitter)
