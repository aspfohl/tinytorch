...
class Linear(minitorch.Module):
    ...
    def forward(self, x):
        num_layers = len(self.weights[0])
        res = []
        for j in range(num_layers):
            s = self.bias[j].value
            for i in range(len(x)):
                s += self.weights[i][j].value * x[i]
            res.append(s)
        return res
...
def test_linear_sanity_check():
    ...
    assert len(mylinear.weights) == 3
    assert len(mylinear.weights[0]) == 5
    assert hasattr(mylinear, "weight_0_0")

    assert len(mylinear.bias) == 5
    assert hasattr(mylinear, "bias_0")

    res = mylinear.forward([1, 2, 3])
    assert len(res) == 5
    for i in range(3):
        assert (
            res[i]
            == mylinear.bias[i].value
            + mylinear.weights[i][0].value
            + 2 * mylinear.weights[i][1].value
            + 3 * mylinear.weights[i][2].value
        )
