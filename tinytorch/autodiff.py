...
class FunctionBase:
    ...
    def chain_rule(cls, ctx, inputs, d_output):
        ...
        """
        backwards = wrap_tuple(cls.backward(ctx, d_output))
        return [
            (i, i.expand(b)) for i, b in zip(inputs, backwards) if not is_constant(i)
        ]
...
def backpropagate(variable, deriv):
    ...
    # Call topological sort to get an order
    sorted_variable = topological_sort(variable)
    derivatives = {}
    derivatives[variable.unique_id] = deriv

    # For each node in order
    for var in sorted_variable:
        # pull a completed Variable and derivative from the queue
        deriv = derivatives[var.unique_id]

        # if the Variable is a leaf, add its final derivative and loop to next var
        if var.is_leaf():
        ...
        for v, v_deriv in var.history.backprop_step(deriv):
            if v.unique_id not in derivatives:
                derivatives[v.unique_id] = v_deriv
            else:
                derivatives[v.unique_id] += v_deriv
