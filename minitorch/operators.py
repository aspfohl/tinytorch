...
def sigmoid(x: float) -> float:
    ...
    Returns:
        float : sigmoid value
    """
    return 1 / (1 + math.exp(-x)) if x >= 0.0 else math.exp(x) / (1 + math.exp(x))


def sigmoid_back(x: float, d) -> float:
    r"If :math:`f = sigmoid` as above, compute d :math:`d \times f'(x)`"
    # http://www.ai.mit.edu/courses/6.892/lecture8-html/sld015.htm
    sig = 1 / (1 + math.exp(-x)) if x >= 0.0 else math.exp(x) / (1 + math.exp(x))
    return d * sig * (1 - sig)
...
def zipWith(fn):
    ...
    def _zipWith(ls1: list, ls2: list) -> list:
